import { motion } from 'framer-motion';
import { Link } from 'react-router-dom';
import { useTheme } from '../../context/ThemeContext';
import { 
  EyeIcon,
  EnvelopeIcon,
  PhoneIcon,
  MapPinIcon,
  ArrowTopRightOnSquareIcon
} from '@heroicons/react/24/outline';

function Footer() {
  const currentYear = new Date().getFullYear();
  const { theme, isDarkMode, animations } = useTheme();
  
  const footerLinks = [
    {
      title: 'Produk',
      links: [
        { name: 'Fitur', href: '/#features' },
        { name: 'Harga', href: '/#pricing' },
        { name: 'FAQ', href: '/#faq' },
        { name: 'Testimonial', href: '/#testimonials' },
      ]
    },
    {
      title: 'Perusahaan',
      links: [
        { name: 'Tentang Kami', href: '/#about' },
        { name: 'Blog', href: '/blog' },
        { name: 'Karir', href: '/careers' },
        { name: 'Kontak', href: '/contact' },
      ]
    },
    {
      title: 'Legal',
      links: [
        { name: 'Kebijakan Privasi', href: '/privacy-policy' },
        { name: 'Syarat & Ketentuan', href: '/terms' },
        { name: 'Kebijakan Cookie', href: '/cookie-policy' },
      ]
    },
  ];
  
  const socialLinks = [
    { 
      name: 'Twitter', 
      icon: 'M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84',
      href: '#'
    },
    { 
      name: 'GitHub', 
      icon: 'M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z',
      href: '#'
    },
    { 
      name: 'Instagram', 
      icon: 'M12.315 2c2.43 0 2.784.013 3.808.06 1.064.049 1.791.218 2.427.465a4.902 4.902 0 011.772 1.153 4.902 4.902 0 011.153 1.772c.247.636.416 1.363.465 2.427.048 1.067.06 1.407.06 4.123v.08c0 2.643-.012 2.987-.06 4.043-.049 1.064-.218 1.791-.465 2.427a4.902 4.902 0 01-1.153 1.772 4.902 4.902 0 01-1.772 1.153c-.636.247-1.363.416-2.427.465-1.067.048-1.407.06-4.123.06h-.08c-2.643 0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 4.902 0 01-1.772-1.153 4.902 4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.024-.06-1.379-.06-3.808v-.63c0-2.43.013-2.784.06-3.808.049-1.064.218-1.791.465-2.427a4.902 4.902 0 011.153-1.772A4.902 4.902 0 015.45 2.525c.636-.247 1.363-.416 2.427-.465C8.901 2.013 9.256 2 11.685 2h.63zm-.081 1.802h-.468c-2.456 0-2.784.011-3.807.058-.975.045-1.504.207-1.857.344-.467.182-.8.398-1.15.748-.35.35-.566.683-.748 1.15-.137.353-.3.882-.344 1.857-.047 1.023-.058 1.351-.058 3.807v.468c0 2.456.011 2.784.058 3.807.045.975.207 1.504.344 1.857.182.466.399.8.748 1.15.35.35.683.566 1.15.748.353.137.882.3 1.857.344 1.054.048 1.37.058 4.041.058h.08c2.597 0 2.917-.01 3.96-.058.976-.045 1.505-.207 1.858-.344.466-.182.8-.398 1.15-.748.35-.35.566-.683.748-1.15.137-.353.3-.882.344-1.857.048-1.055.058-1.37.058-4.041v-.08c0-2.597-.01-2.917-.058-3.96-.045-.976-.207-1.505-.344-1.858a3.097 3.097 0 00-.748-1.15 3.098 3.098 0 00-1.15-.748c-.353-.137-.882-.3-1.857-.344-1.023-.047-1.351-.058-3.807-.058zM12 6.865a5.135 5.135 0 110 10.27 5.135 5.135 0 010-10.27zm0 1.802a3.333 3.333 0 100 6.666 3.333 3.333 0 000-6.666zm5.338-3.205a1.2 1.2 0 110 2.4 1.2 1.2 0 010-2.4z',
      href: '#'
    },
    { 
      name: 'Facebook', 
      icon: 'M22.675 0h-21.35c-.732 0-1.325.593-1.325 1.325v21.351c0 .731.593 1.324 1.325 1.324h11.495v-9.294h-3.128v-3.622h3.128v-2.671c0-3.1 1.893-4.788 4.659-4.788 1.325 0 2.463.099 2.795.143v3.24l-1.918.001c-1.504 0-1.795.715-1.795 1.763v2.313h3.587l-.467 3.622h-3.12v9.293h6.116c.73 0 1.323-.593 1.323-1.325v-21.35c0-.732-.593-1.325-1.325-1.325z',
      href: '#'
    },
  ];

  return (
    <footer className={`relative overflow-hidden ${
      isDarkMode 
        ? 'bg-gray-900 text-gray-300' 
        : 'bg-gradient-to-br from-indigo-600 to-purple-600 text-white'
    }`}>
      {/* Wave decoration */}
      <div className="absolute top-0 left-0 right-0 h-6 overflow-hidden">
        <svg 
          className="absolute bottom-0 w-full"
          viewBox="0 0 1200 120"
          preserveAspectRatio="none"
        >
          <motion.path 
            d="M321.39,56.44c58-10.79,114.16-30.13,172-41.86,82.39-16.72,168.19-17.73,250.45-.39C823.78,31,906.67,72,985.66,92.83c70.05,18.48,146.53,26.09,214.34,3V0H0V27.35A600.21,600.21,0,0,0,321.39,56.44Z"
            initial={{ pathLength: 0, opacity: 0 }}
            animate={{ 
              pathLength: 1, 
              opacity: 1,
              transition: { duration: 1.5, ease: "easeInOut" }
            }}
            fill={isDarkMode ? "rgba(31, 41, 55, 0.5)" : "rgba(255, 255, 255, 0.1)"}
          />
        </svg>
      </div>

      <div className="max-w-7xl mx-auto pt-16 pb-8 px-4 sm:px-6 lg:px-8">
        <div className="xl:grid xl:grid-cols-5 xl:gap-8">
          {/* Brand */}
          <motion.div 
            className="space-y-6 xl:col-span-2"
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true, amount: 0.3 }}
            variants={animations.fadeInUp}
            custom={0}
          >
            <Link to="/" className="flex items-center">
              <EyeIcon className={`h-8 w-8 ${isDarkMode ? 'text-blue-400' : 'text-blue-200'}`} />
              <span className={`ml-2 text-xl font-bold ${isDarkMode ? 'text-white' : 'text-white'}`}>
                RetinaScan
              </span>
            </Link>
            
            <motion.p 
              className={`max-w-xs text-sm ${isDarkMode ? 'text-gray-400' : 'text-blue-100'}`}
              variants={animations.fadeInUp}
              custom={0.2}
            >
              Deteksi dini retinopati diabetik dengan teknologi AI canggih. Kami membantu dokter dan pasien untuk mendiagnosa lebih cepat dan akurat.
            </motion.p>
            
            <motion.div 
              className="flex space-x-5"
              variants={animations.fadeInUp}
              custom={0.4}
            >
              {socialLinks.map((social) => (
                <motion.a
                  key={social.name}
                  href={social.href}
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`${isDarkMode ? 'text-gray-400 hover:text-white' : 'text-blue-200 hover:text-white'}`}
                  whileHover={{ scale: 1.2 }}
                  whileTap={{ scale: 0.9 }}
                  aria-label={social.name}
                >
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    <path d={social.icon} />
                  </svg>
                </motion.a>
              ))}
            </motion.div>
          </motion.div>
          
          {/* Links */}
          <div className="mt-12 grid grid-cols-2 gap-8 xl:mt-0 xl:col-span-3">
            <div className="md:grid md:grid-cols-3 md:gap-8">
              {footerLinks.map((group, groupIdx) => (
                <motion.div 
                  key={group.title}
                  initial="hidden"
                  whileInView="visible"
                  viewport={{ once: true, amount: 0.3 }}
                  variants={animations.fadeInUp}
                  custom={0.2 + groupIdx * 0.1}
                  className="mt-12 md:mt-0"
                >
                  <h3 className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-white'} tracking-wider uppercase`}>
                    {group.title}
                  </h3>
                  <motion.ul 
                    className="mt-4 space-y-3"
                    variants={animations.container}
                  >
                    {group.links.map((link, linkIdx) => (
                      <motion.li key={link.name} variants={animations.item}>
                        <Link 
                          to={link.href} 
                          className={`text-sm ${
                            isDarkMode 
                              ? 'text-gray-400 hover:text-white' 
                              : 'text-blue-100 hover:text-white'
                          } hover:translate-x-1 inline-flex items-center transition-all duration-200`}
                  >
                    {link.name}
                        </Link>
                      </motion.li>
                    ))}
                  </motion.ul>
                </motion.div>
              ))}
            </div>
          </div>
        </div>
          
          {/* Contact Info */}
          <motion.div 
          className="border-t border-opacity-20 mt-12 pt-8"
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, amount: 0.1 }}
          variants={animations.fadeInUp}
          custom={0.6}
          style={{ borderColor: isDarkMode ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.2)' }}
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-y-6 md:gap-x-8">
            <motion.div 
              className="flex items-center"
              whileHover={{ x: 5 }}
              >
              <EnvelopeIcon className="h-5 w-5 mr-3 flex-shrink-0" />
              <a 
                href="mailto:info@retinascan.com" 
                className="text-sm hover:underline"
              >
                info@retinascan.com
              </a>
            </motion.div>
            
            <motion.div 
              className="flex items-center"
              whileHover={{ x: 5 }}
            >
              <PhoneIcon className="h-5 w-5 mr-3 flex-shrink-0" />
              <a 
                href="tel:+62211234567" 
                className="text-sm hover:underline"
              >
                +62 21 1234 567
              </a>
            </motion.div>
            
            <motion.div 
              className="flex items-center"
              whileHover={{ x: 5 }}
            >
              <MapPinIcon className="h-5 w-5 mr-3 flex-shrink-0" />
              <span className="text-sm">
                Jakarta, Indonesia
              </span>
          </motion.div>
        </div>
        </motion.div>
        
        {/* Copyright */}
        <motion.div 
          className="mt-12 border-t border-opacity-20 pt-8 flex flex-col md:flex-row justify-between items-center"
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, amount: 0.1 }}
          variants={animations.fadeInUp}
          custom={0.8}
          style={{ borderColor: isDarkMode ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.2)' }}
        >
          <p className="text-xs">
            &copy; {currentYear} RetinaScan. Hak Cipta Dilindungi.
          </p>
          
          <div className="mt-4 md:mt-0 flex space-x-6">
            <motion.a 
              href="https://www.termsfeed.com/live/privacy-policy" 
              target="_blank"
              rel="noopener noreferrer"
              className="text-xs flex items-center hover:underline"
              whileHover={{ x: 2 }}
            >
              Kebijakan Privasi
              <ArrowTopRightOnSquareIcon className="ml-1 h-3 w-3" />
            </motion.a>
            
            <motion.a 
              href="https://www.termsfeed.com/live/terms-conditions" 
              target="_blank"
              rel="noopener noreferrer"
              className="text-xs flex items-center hover:underline"
              whileHover={{ x: 2 }}
            >
              Syarat & Ketentuan
              <ArrowTopRightOnSquareIcon className="ml-1 h-3 w-3" />
            </motion.a>
          </div>
        </motion.div>
      </div>
    </footer>
  );
}

export default Footer;